...
//	    buildMapperXml(Manager.class,"com.ziteng.dao.user","E:\\program\\紫藤\\develop\\kk\\config\\mapper","");
	 ...
	 */
	public static void buildMapperXml(Class className,String daoPackage,String outputPath,String tablePrefix){
		bulidGeneralMapper(className,daoPackage,outputPath,tablePrefix);
	}
	 ...
	 */
	private static void bulidGeneralMapper(Class className,String daoPackage,String outputPath,String tablePrefix){
		buildXml(className,daoPackage,outputPath,tablePrefix);
	}
	
	
	private static void buildXml(Class className,String daoPackage,String outputPath,String tablePrefix){
		...
		//创建insert
		root.add(buildInsert(className,tablePrefix));
		//创建update
	...
	private static Element buildUpdate(Class className,String tablePrefix){
		...
		for(int i=0;i<field.length;i++){
			...
			if(field[i].getModifiers()==2&&!propertyName.equals("id")&&!propertyName.equals(idPropertyName)&&!propertyName.equals("")){
				sbf.append(changeProperty2Column(propertyName)).append("=").append("#{").append(propertyName).append("}");
				if(i<field.length-1){
					sbf.append(",");
				}
				sbf.append("\n\t\t");
	 ...
	 */
	private static Element buildInsert(Class className,String tablePrefix){
